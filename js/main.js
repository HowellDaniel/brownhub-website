(function () {
  ...
  if (!prefersReducedMotion) {
    ...
    style.textContent = `
      .interactive-background { position:fixed; inset:0; z-index:-1; pointer-events:none; overflow:hidden; background:radial-gradient(circle at var(--pointer-x,50%) var(--pointer-y,35%),rgba(var(--accent-rgb),.14),transparent 28rem),radial-gradient(circle at 85% 15%,rgba(var(--accent-2-rgb),.12),transparent 24rem); }
      ...
      .interactive-background::before { left:8%; top:18%; background:var(--accent); }
      .interactive-background::after { right:8%; bottom:12%; background:var(--accent-2); opacity:.09; animation-delay:-6s; }
